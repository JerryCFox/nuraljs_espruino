  ...
  logger.init({                                       //ENABLE LOGGER
    ...
    wifimodule.init({                                 //Enable WIFI
      ...
      webcore.init({                                  //Enable Webcore
        ...
        memorylogger=setInterval(function(){logger.log(""+getTime()+" RAM:"+process.memory().free);},30000);
        connectToMPU(function(err,status){
          gravitylogger=setInterval(function(){
            logger.log(mpu.getGravity()[2]);
          },500);
        });
      });
    });
